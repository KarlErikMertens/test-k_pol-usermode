 ...
 *(".kobject_data.rodata*")
 ...
 ASSERT(_kobject_rodata_area_used <= 272,
"scripts/gen_kobject_placeholders.py did not reserve enough space for kobject rodata."
 );
 . = MAX(., _kobject_rodata_area_start + 272);
 } > RAM
 . = ALIGN(0x1000);
 __rodata_region_end = .;
 ...
 *(".kobject_data.sdata*")
 ...
 ASSERT(_kobject_data_area_used <= 1600,
"scripts/gen_kobject_placeholders.py did not reserve enough space for kobject data."
 );
 . = MAX(., _kobject_data_area_start + 1600);
        } > RAM
 . = ALIGN(0x1000);
 _image_ram_end = .;
 z_mapped_end = .;
