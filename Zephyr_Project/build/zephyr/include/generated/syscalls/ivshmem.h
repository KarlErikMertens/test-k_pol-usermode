...
extern int z_impl_ivshmem_get_semaphore(const struct device * dev);

__pinned_func
static inline int ivshmem_get_semaphore(const struct device * dev)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		/* coverity[OVERRUN] */
		return (int) arch_syscall_invoke1(*(uintptr_t *)&dev, K_SYSCALL_IVSHMEM_GET_SEMAPHORE);
	}
#endif
	compiler_barrier();
	return z_impl_ivshmem_get_semaphore(dev);
}
